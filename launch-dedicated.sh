...
#
# Tweaked to work with the `winportable` release.  Based on 1.08.2.  Changes are highlighted
...
cd "${TEMPLATE_ROOT}"
# CHANGED: The dlls are in the same directory
if [ ! -f "${ENGINE_DIRECTORY}/OpenRA.Server.dll" ] || [ "$(cat "${ENGINE_DIRECTORY}/VERSION")" != "${ENGINE_VERSION}" ]; then
...
while true; do
     # CHANGED: Tweak locations of dll and EngineDir
     MOD_SEARCH_PATHS="${MOD_SEARCH_PATHS}" ${RUNTIME_LAUNCHER} OpenRA.Server.dll Engine.EngineDir="." Game.Mod="${LAUNCH_MOD}" \
     Server.Name="${NAME}" \
